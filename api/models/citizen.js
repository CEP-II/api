...
const citizenSchema = mongoose.Schema({
  ...
  birthdate: {
    // <-- better than to store age, which changes over time. Have to compute age from this tho.
    type: Date,
    required: true,
  ...
  address: {
    type: {
      // add country field if we want non-Denmark addresses (would also have to change regular expressions)
      postal: Number,
      street: String,
      city: String,
  ...
  phone: {
    // might not have phone, not required
    type: Number,
    unique: true,
